...
import { Locale, DEFAULT_LOCALE, isPilotTool } from '@/lib/i18n';
...
export default function RelatedTools({ relatedSlugs, locale = DEFAULT_LOCALE }: RelatedToolsProps) {
  // Find matching tool definitions
  const relatedList = toolsMaster.filter((t) => relatedSlugs.includes(t.slug)).slice(0, 4);

  if (relatedList.length === 0) return null;

  const common = loadTranslations(locale);
  const headingText = t(common, 'tool.relatedTools') || 'Related Tools';
    ...
    <div className="my-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-5">
        {relatedList.map((tool) => {
          ...
          const href =
            locale !== DEFAULT_LOCALE && isPilot
              ? `/${locale}/tools/${tool.slug}`
              : `/tools/${tool.slug}`;

          const content =
            locale !== DEFAULT_LOCALE && isPilot
              ? getToolTranslation(tool.slug, locale)
              : null;

          const toolName = content?.name || tool.name;
          const toolDesc = content?.description || tool.description;
